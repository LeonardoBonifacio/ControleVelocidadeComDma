...
#define NUM_VALORES 128 // Redula o brilho final do LED
...
int main()
{
    // Configura botão BOOTSEL
    gpio_init(BOOTSEL_PIN);
    gpio_set_dir(BOOTSEL_PIN, GPIO_IN);
    gpio_pull_up(BOOTSEL_PIN);
    gpio_set_irq_enabled_with_callback(BOOTSEL_PIN, GPIO_IRQ_EDGE_FALL, true, &gpio_irq_handler);
    ...
    dma_channel_config c_cont = dma_channel_get_default_config(ctrl_chan);
    channel_config_set_transfer_data_size(&c_cont, DMA_SIZE_32);
    channel_config_set_read_increment(&c_cont, false);
    channel_config_set_write_increment(&c_cont, false);
    channel_config_set_chain_to(&c_cont, data_chan); // Controle aciona dados
    dma_channel_configure(ctrl_chan, &c_cont,
